...
/*
 * Body pool's reduce the pressure on the iommu TLB by allocating a single
 * large area of DMA memory and allocating it as a pool of display list bodies
 */
struct vsp1_dl_body_pool *
vsp1_dl_body_pool_alloc(struct vsp1_device *vsp1, unsigned int num_bodies,
...
void vsp1_dl_body_pool_free(struct vsp1_dl_body_pool *pool)
{
	if (!pool)
		return;
 ...
 * list. The caller must not modify the body after this call, but can retain
					...
					unsigned int prealloc)
{
	...
	dlm->pool = vsp1_dl_body_pool_alloc(vsp1, prealloc,
...
void vsp1_dlm_destroy(struct vsp1_dl_manager *dlm)
{
	...
	vsp1_dl_body_pool_free(dlm->pool);
}
