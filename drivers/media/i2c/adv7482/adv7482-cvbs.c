/*
 * Driver for Analog Devices ADV7482 8 channel analog front end (AFE) receiver
...
int adv7482_sdp_probe(struct adv7482_state *state)
{
	...
	state->sdp.sd.port = ADV7482_PORT_TXB;

	state->sdp.pads[ADV7482_SDP_SINK].flags = MEDIA_PAD_FL_SINK;
	state->sdp.pads[ADV7482_SDP_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
