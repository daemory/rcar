...
struct adv748x_register {
	const char *name;
	u8 default_addr;
};

static const struct adv748x_register adv748x_secondary_names[] = {
...
static int adv748x_initialise_clients(struct adv748x_state *state)
{
	...
	for (i = ADV748X_PAGE_DPLL; i < ADV748X_PAGE_MAX; ++i) {
		state->i2c_clients[i] = i2c_new_secondary_device(
				state->client,
				adv748x_secondary_names[i].name,
				adv748x_secondary_names[i].default_addr);
